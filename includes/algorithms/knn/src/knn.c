...
dataset *get_dataset_subset(dataset *data, int start, int end) {
  dataset *subset = malloc(sizeof(dataset));
  subset->size = end - start;
  subset->samples = malloc(subset->size * sizeof(Point));
  for (int i = start; i < end; i++) {
    subset->samples[i - start] = data->samples[i];
  }
  return subset;
}
...
void divide_dataset(dataset *data, t_knn *knn, float training_ratio) {
  ...
  knn->training->samples = malloc(training_count * sizeof(Point));
  knn->testing->size = testing_size;
  knn->testing->samples = malloc(testing_size * sizeof(Point));
  // Copy samples from the original dataset to the training and testing datasets
  for (int i = 0; i < training_count; i++) {
    knn->training->samples[i] = data->samples[i];
  }
  for (int i = 0; i < testing_size; i++) {
    knn->testing->samples[i] = data->samples[i + training_count];
  }
}
...
size_t kp_predict(klass_predictor *kp, Point *text, size_t k) {
  ...
  // print the neighbourhood
  // for (int i = 0; i < k; ++i) {
  //   printf("[ %-10.10s ] [ %2.2f ]\n", kp->neighbours[i].point->label,
  //          kp->neighbours[i].distance);
  // }
